        ...
        if(err) {
            ...
            if(req.file == undefined) {
                ...
                try {
                    ...
                    let newObj = Object.assign(req.body, { docPath: req.body.documentPath = img_url});
                    ...
                    const result = await doctorModel.create(newObj);
                    const user = {
...
const getData = async (req, res) => {
    try {
        const result = await doctorModel.find().populate('role');
        res.status(200).json({
